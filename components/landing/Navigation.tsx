  ...
  return (
    ...
    >
      ...
      >
        <a
          href="#hero"
          className="shrink-0 text-sm font-black uppercase tracking-[0.24em] text-cream"
        >
          Reframe Studio
        </a>
        <div className="flex min-w-0 items-center gap-1 overflow-x-auto rounded-full border border-white/[0.055] bg-white/[0.025] p-1 shadow-[0_16px_42px_rgba(0,0,0,0.18)]">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={`#${item.href}`}
              className="whitespace-nowrap rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-cream-muted/85 transition duration-500 hover:bg-white/[0.05] hover:text-cream"
            >
              {item.label}
            </a>
          ))}
